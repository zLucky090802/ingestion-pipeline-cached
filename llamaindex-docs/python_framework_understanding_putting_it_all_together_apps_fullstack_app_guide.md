...
constfetchDocuments=async ():PromiseDocument[]> => {




constresponse=awaitfetch("http://localhost:5601/getDocuments", {
...
constdocumentList= (await response.json()) asDocument[];
...
constqueryIndex=async (query:string):PromiseQueryResponse=> {




constqueryURL=newURL("http://localhost:5601/query?text=1");
...
constresponse=awaitfetch(queryURL, { mode: "cors" });
...
constqueryResponse= (await response.json()) asQueryResponse;
...
constinsertDocument=async (file:File) => {
...
constresponse=awaitfetch("http://localhost:5601/uploadFile", {
