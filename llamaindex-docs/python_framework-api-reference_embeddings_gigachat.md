...
```
 | 
```
classGigaChatEmbedding(BaseEmbedding):
"""
    GigaChat encoder class for generating embeddings.
...
"""
        ...
        try:
            ...
            raise ValueError(f"GigaChat client failed to initialize. Error: {e}") frome
...
"""
    ...
    def_get_query_embedding(self, query: List[str]) -> List[float]:
"""
    ...
    async def_aget_query_embedding(self, query: List[str]) -> List[float]:
"""
